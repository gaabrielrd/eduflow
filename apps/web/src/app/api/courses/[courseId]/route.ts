...
import type { UpdateCoursePayload } from "@/lib/courses/course-types";
...
export async function GET(_: Request, context: RouteContext) {
  ...
  try {
    const course = await externalApiRequest({
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const { courseId } = await context.params;
  const payload = (await request.json()) as UpdateCoursePayload;
