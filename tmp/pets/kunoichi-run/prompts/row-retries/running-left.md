Create Codex pet row `running-left` for `kunoichi`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: female ninja, chibi style, anime figure inspired, compact whole-body silhouette, black hair in a short high ponytail, dark indigo ninja outfit with red scarf accents, fingerless gloves, soft boots, tiny kunai sheath at the waist, expressive large eyes, clean cel-shaded figure-like finish, cute but agile. Preserve silhouette, face, palette, material, proportions, markings, and props.
